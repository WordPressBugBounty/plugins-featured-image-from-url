...
require_once __DIR__ . '/class-fifu-settings-strings.php';
require_once __DIR__ . '/class-fifu-meta-box-strings.php';

/**
 * Façade for admin UI strings.
 *
 * @package Fifu_Free
 */
class Fifu_Admin_Strings {

    /**
     * Return the complete settings strings payload handled by the legacy loader.
     *
     * @return array<string, mixed>
     */
    public static function get_settings_strings(): array {
        $fifu = array();
        ...
        // options
        $fifu['options'] = Fifu_Settings_Strings::get_option_labels();
        ...
        // messages
        $fifu['message'] = Fifu_Settings_Strings::get_messages();

        // tabs
        $fifu['tab'] = Fifu_Settings_Strings::get_tab_labels();
        ...
        $fifu['title']['vimeo-options'] = function () {
            _e("Vimeo options", FIFU_SLUG);
        };
        $fifu['title']['youtube-options'] = function () {
            _e("YouTube options", FIFU_SLUG);
        };
        ...
        $fifu['vimeo']['mp4-desc'] = function () {
            _e("When an access token is provided, FIFU can use Vimeo MP4 video files instead of Vimeo streaming.", FIFU_SLUG);
        };
        $fifu['vimeo']['tab']['access-token'] = function () {
            _e("Access token", FIFU_SLUG);
        };
        $fifu['vimeo']['access-token'] = function () {
            _e("access token", FIFU_SLUG);
        };
        $fifu['vimeo']['tab']['resolution'] = function () {
            _e("Video resolution", FIFU_SLUG);
        };
        $fifu['vimeo']['mp4-rendition'] = function () {
            _e("preferred video resolution", FIFU_SLUG);
        };
        $fifu['vimeo']['mp4-resolution-desc'] = function () {
            _e("FIFU will use the selected MP4 resolution when available. If it is not available, FIFU will use the closest lower resolution, or the closest higher one if no lower file exists.", FIFU_SLUG);
        };
        $fifu['vimeo']['access-token-info'] = function () {
            _e("Required scopes: public, private, video_files.", FIFU_SLUG);
        };
        $fifu['vimeo']['access-token-help'] = function () {
            _e("How to generate a personal access token", FIFU_SLUG);
        };

        // help texts
        foreach ( Fifu_Settings_Strings::get_help_texts() as $key => $value ) {
            $fifu[ $key ] = $value;
        }
    ...
    ", FIFU_SLUG);
        ...
        $fifu['share']['desc'] = function () {
            _e("Automatically share published posts when they are created or updated on WordPress to social media. The rate limit is 16 posts per day for each connected social media platform.", FIFU_SLUG);
        };
        ...
        _e("Skip media tags containing these keywords:", FIFU_SLUG);
        };
        ...
        $fifu['debug']['tables'] = function () {
            _e("It also enables troubleshooting tools that help collect diagnostic data from posts, products, categories, and media records.", FIFU_SLUG);
        };
        ...
        $fifu['autoplay']['front_all'] = function () {
            _e('play all homepage videos simultaneously', FIFU_SLUG);
        };
        ...
        $fifu['background']['desc'] = function () {
            _e("These options are only for Vimeo videos.", FIFU_SLUG);
        };
        $fifu['background']['tab']['background'] = function () {
            _e("Background video", FIFU_SLUG);
        };
        $fifu['background']['tab']['mp4'] = function () {
            _e("mp4", FIFU_SLUG);
        };
        $fifu['background']['video-desc'] = function () {
        ...
        $fifu['privacy']['desc'] = function () {
            _e("These options are only for YouTube videos.", FIFU_SLUG);
        };
        $fifu['privacy']['label'] = function () {
            _e("privacy enhanced mode", FIFU_SLUG);
        };
        $fifu['privacy']['info'] = function () {
            _e("Prevents views of embedded YouTube content from affecting the viewer's browsing experience on YouTube.", FIFU_SLUG);
        };
